...
def koop_mixup_data_aug(args, 
    ...
    if use_all:
        x,u,config = data_generate_all(args, env, dataset, trial_len, random_seed)

    else:
        x,u,config = data_generate(args, env, dataset, trial_len, random_seed)
    ...
    with tf.Session() as sess:
        ...
        if dvk_model_train:
            ...
            saver.restore(sess, os.path.join(args.save_dir, dvk_model_dir + "dvk.ckpt-{}".format(0))) # load specific learned dvk model when skip = True
...
def data_generate(args, env, dataset, trial_len, random_seed):
    ...
    seq_length = args.seq_length*2 ## for DVK learning
    ...
    for i in range(n_trials):
        ...
        sample_num += n_subseq
        idx += 1
        if sample_num > args.n_subseq: # You can change this number to whatever you want
            break
...
def data_generate_all(args, env, dataset, trial_len, random_seed):
    ...
    config = dict(
        env_name = str(env).split()[1].lstrip("<"),
        # env_name = str(env).split("<")[3].rstrip(">"),
        seq_length = seq_length,
    )
